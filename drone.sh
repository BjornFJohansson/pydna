...
then
    ...
    if [ "$branch" = "py2" ]
    then
        conda create -q -n    pydnabuild python=2.7
        conda create -q -y -n pipbuild   python=2.7
    elif [ "$branch" = "py3" ]||[ "$branch" = "py3dev" ]
    then
        conda create -q -n    pydnabuild python=3.5
    ...
    then
        ...
        anaconda -q -t $TOKEN upload $pth --label test
        python setup.py register -r pypitest
        twine upload -r pypitest dist/* --skip-existing
    else
